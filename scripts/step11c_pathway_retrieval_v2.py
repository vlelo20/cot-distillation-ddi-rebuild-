#!/usr/bin/env python3
"""
Step 11c v2: Pathway-based retrieval with asymmetric handling.

Difference from v1:
- pair_sim now handles cases where one of the four drug feature sets is
  empty. Falls back to single-sided best-match similarity * a penalty
  (default 0.5). Recovers the ~85% of empty results in v1 that came from
  query pairs where exactly one drug had pathway data.
- Output filename ends in `_v2.json` to coexist with v1 results.

Run alongside v1 to compare empty rate, sample retrievals, and downstream
prompt quality on the same pilot query set.
"""
...
def pair_sim(
    ...
    asymmetric_penalty: float = 0.5,
) -> float:
    """
    Symmetric pair similarity. Three cases:

    (1) Full pair (all four feature sets nonempty): try both pairings
        (qa<->ca, qb<->cb) and (qa<->cb, qb<->ca), take the geometric
        mean of the better one. Geometric mean keeps the score in [0,1]
        and stays linearly comparable to per-drug weighted Jaccard.

    (2) Asymmetric (at least one query or candidate drug lacks features):
        compute single-sided similarity between the side(s) that have
        features, and apply `asymmetric_penalty` to reflect that one
        drug's mechanistic profile is unknown rather than matched.

    (3) Both query drugs lack features OR both candidate drugs lack
        features: return 0 (no signal).
    """
    has_qa, has_qb = bool(qa), bool(qb)
    has_ca, has_cb = bool(ca), bool(cb)
    if not (has_qa or has_qb) or not (has_ca or has_cb):
        return 0.0

    if has_qa and has_qb and has_ca and has_cb:
        s1 = drug_sim(qa, ca) * drug_sim(qb, cb)
        s2 = drug_sim(qa, cb) * drug_sim(qb, ca)
        return (s1 if s1 >= s2 else s2) ** 0.5

    # Asymmetric: take the best single-drug match across the side(s) that have features.
    best = 0.0
    if has_qa:
        if has_ca: best = max(best, drug_sim(qa, ca))
        if has_cb: best = max(best, drug_sim(qa, cb))
    if has_qb:
        if has_ca: best = max(best, drug_sim(qb, ca))
        if has_cb: best = max(best, drug_sim(qb, cb))
    return best * asymmetric_penalty
...
def retrieve(
    ...
    # Pre-compute per-drug sims and feature-presence flags against unique
    # candidate drugs (avoids redoing the same Jaccard / lookup repeatedly).
    ...
    has_feat: Dict[str, bool] = {}
    for d in unique_drugs:
        df = drug_features.get(d, set())
        has_feat[d] = bool(df)
        sim_qa[d] = drug_sim(qa, df) if has_qa else 0.0
        sim_qb[d] = drug_sim(qb, df) if has_qb else 0.0
    ...
    for idx in candidates:
        ...
        has_ca, has_cb = has_feat[ca], has_feat[cb]
        if not (has_ca or has_cb):
            continue  # candidate has no features at all

        if has_qa and has_qb and has_ca and has_cb:
            # Full-pair: geometric mean of the better drug-to-drug pairing.
            s1 = sim_qa[ca] * sim_qb[cb]
            s2 = sim_qa[cb] * sim_qb[ca]
            s = (s1 if s1 >= s2 else s2) ** 0.5
        else:
            # Asymmetric on at least one side: best single-drug match * penalty.
            best = 0.0
            if has_qa:
                if has_ca: best = sim_qa[ca] if sim_qa[ca] > best else best
                if has_cb: best = sim_qa[cb] if sim_qa[cb] > best else best
            if has_qb:
                if has_ca: best = sim_qb[ca] if sim_qb[ca] > best else best
                if has_cb: best = sim_qb[cb] if sim_qb[cb] > best else best
            s = best * asymmetric_penalty
...
def main():
    ...
    ap.add_argument("--asymmetric-penalty", type=float, default=0.5,
                    help="Score multiplier for asymmetric matches "
                         "(one drug lacks features). 0.5 = down-weight by half.")
    ...
    for target in targets:
        ...
        for i, q in enumerate(queries):
            top = retrieve(
                ...
                asymmetric_penalty=args.asymmetric_penalty,
            )
            if not top:
                empty += 1
            results.append(top)
        ...
        out_path = args.out_dir / f"retrieved_examples_pathway_{target}_{suffix}_v2.json"
        log.info(
